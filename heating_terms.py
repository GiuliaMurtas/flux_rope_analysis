...
for t in range(0,20):
    ...
    ohm_element=np.sum(ds['eta'][7:874,7:502,7:502]*jtot2[7:874,7:502,7:502])*dxm*dym*dzm
...
for t in range(0,18):
    ...
    vD = np.power((ds['vx_n'][7:874,7:502,7:502]-ds['vx_p'][7:874,7:502,7:502]),2.0) \
    + np.power((ds['vy_n'][7:874,7:502,7:502]-ds['vy_p'][7:874,7:502,7:502]),2.0) \
    + np.power((ds['vz_n'][7:874,7:502,7:502]-ds['vz_p'][7:874,7:502,7:502]),2.0)
    
    alpha = 1.0*np.sqrt(0.5*(Tn+Tp))*np.sqrt(1 + (9*np.pi/64)*(5.0/6.0)*(vD/(Tn+Tp)))

    fric_element = np.sum(0.5*alpha*ds['ro_n'][7:874,7:502,7:502]*ds['ro_p'][7:874,7:502,7:502]*vD)*dxm*dym*dzm
...
for t in range(0,18):
    ...
    vp2 = np.power(ds['vx_p'][7:874,7:502,7:502],2.0) + np.power(ds['vy_p'][7:874,7:502,7:502],2.0) \
    + np.power(ds['vz_p'][7:874,7:502,7:502],2.0)
    vn2 = np.power(ds['vx_n'][7:874,7:502,7:502],2.0) + np.power(ds['vy_n'][7:874,7:502,7:502],2.0) \
    + np.power(ds['vz_n'][7:874,7:502,7:502],2.0)
...
ax.plot(time1,fric1,color='red', linestyle='dashed', label = "FH$_1$ (P1)")
ax.plot(time2,fric2,color='red', linestyle='dotted', label = "FH$_1$ (P2)")
ax.plot(time1,fric1,color='blue', linestyle='dashed', label = "FH$_2$ (P1)")
ax.plot(time2,fric2,color='blue', linestyle='dotted', label = "FH$_2$ (P2)")
